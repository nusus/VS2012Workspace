...
GLfloat rotationTriangle = 7.0f;
GLfloat rotationQuad = -7.0f;
...
GLfloat g_lightAmbient[] = {0.5f, 0.5f, 0.5f, 1.0f};
GLfloat g_lightDiffuse[] = {1.0f, 1.0f, 1.0f, 1.0f};
GLfloat g_lightPosition[] = {0.0f, 0.0f, 2.0f, 1.0f};

GLuint g_filter;
GLuint g_texture[3];

AUX_RGBImageRec* LoadBMP(const char* pszFileName)
{
	FILE* file = NULL; 
	if (!pszFileName)
	{
	...
	fopen_s(&file,pszFileName, "r");
	if (file)
	{
		fclose(file);
		return auxDIBImageLoad(pszFileName);
	}
	return NULL;
}
...
int LoadGLTextures()
{
	...
	if (textureImage[0] = LoadBMP("../resource/zengli.bmp"));
	{
		status = TRUE;
...
int DrawGLScene(GLvoid)									// 从这里开始进行所有的绘制
{
	...
	glBindTexture(GL_TEXTURE_2D, g_texture[g_filter]);
